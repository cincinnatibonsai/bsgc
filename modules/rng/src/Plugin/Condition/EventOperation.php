...
class EventOperation extends ConditionPluginBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['description']['#markup'] = $this->t('For %operation operations.', [
  ...
  public function summary() {
    ...
    $vars = [
      '@operations' => count($operations) > 1 ? implode(', ', $operations) : reset($operations),
    ];
    if ($this->isNegated()) {
      return t('Logged-in user does not have access to operations on event: @operations.', $vars);
    }
    else {
      return t('Logged-in user has access to operations on event: @operations.', $vars);
    }
  }
