...
class EventMeta implements EventMetaInterface {
  ...
  public function removeRegistrationType($registration_type_id) {
    ...
    foreach ($registration_types->getValue() as $key => $value) {
      // @todo fails if same registration is referenced more than once.
  ...
  public function hasCapacity(): bool {
    $capacity = $this->remainingRegistrantCapacity();
    if ($capacity == EventMetaInterface::CAPACITY_UNLIMITED) {
      return TRUE;
    }
    return $capacity > 0;
   ...
   */
  protected function countRngReferenceableEntities(string $entity_type_id, array $bundles = []) {
