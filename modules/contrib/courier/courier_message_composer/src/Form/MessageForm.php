...
class MessageForm extends FormBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('identity')) {
      $identity = NULL;
      [$entity_type, $entity_id] = explode(':', $form_state->getValue('identity'));
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($mqi instanceof MessageQueueItemInterface) {
      $this->messenger()->addMessage($this->t('Message queued for delivery.'));
    }
    else {
      $this->messenger()->addMessage($this->t('Failed to send message'), 'error');
    }
  }
